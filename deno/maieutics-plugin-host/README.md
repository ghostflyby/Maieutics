...
plugin export subpath, scans each worker for extension point marker symbols, and bridges
`extension.*` bus messages between the kernel and the workers.
...
- `--allow-read` — the plugin configuration file, the materialized module directory, every plugin
  directory, the control channel socket path, and plugin-declared read paths.
- `--allow-write` — the control channel socket path plus plugin-declared write paths
  (`Deno.createHttpClient` probes socket write access).
- `--allow-net` — `localhost` plus `unix:<socket>` for the control channel proxy, plus
  plugin-declared net domains.
...
exceed the host process grants (Deno enforces this). Any change to environment, network, or
filesystem behavior must update this list in the same change.
