...
class PluginWorker {
  ...
  constructor(plugin: PluginConfig, exportName: string, options: HostOptions) {
  ...
  async #startWorker(): Promise<void> {
    ...
    if (entryUrl === undefined) {
      throw new Error(`Worker '${this.exportName}' of plugin '${this.pluginId}' has no entry URL.`);
    }
    const worker = new Worker(this.#options.workerEntryUrl, {
      type: "module",
      deno: { permissions: buildWorkerPermissions(this.#plugin, this.#options) },
    });
    this.#worker = worker;
    const ready = new Promise<void>((resolve, reject) => {
      const timeout = setTimeout(() => {
        worker.terminate();
        reject(
          new Error(`Plugin worker '${this.pluginId}/${this.exportName}' did not become ready.`),
        );
      ...
      worker.onmessage = (event: MessageEvent): void => {
        ...
        if (frame.type === "ready") {
          ...
          resolve();
        } else if (frame.type === "error" && (frame.id === null || frame.id === undefined)) {
          clearTimeout(timeout);
          reject(new Error(`Plugin worker failed to initialize: ${JSON.stringify(frame)}`));
        }
      };
  ...
  async invoke(extensionPoint: string, request: unknown): Promise<unknown> {
    ...
    if (worker === undefined) {
      throw new Error(`Plugin worker '${this.pluginId}/${this.exportName}' is not running.`);
    }
    const id = crypto.randomUUID();
    const done = new Promise<unknown>((resolve, reject) => {
      const timer = setTimeout(() => {
        this.#pending.delete(id);
        reject(new Error(`Extension point '${extensionPoint}' timed out.`));
      }, this.#options.invokeTimeoutMs ?? DEFAULT_INVOKE_TIMEOUT_MS);
...
export class PluginHost {
  ...
  constructor(options: HostOptions) {
    ...
    for (const plugin of options.plugins) {
      for (const workerConfig of plugin.workers) {
        const worker = new PluginWorker(plugin, workerConfig.exportName, options);
        this.#workers.set(workerKey(plugin.id, workerConfig.exportName), worker);
      }
    }
  }
  ...
  async invoke(
    ...
    if (worker === undefined) {
      throw new Error(`No worker for plugin '${pluginId}' export '${exportName}'.`);
    }
