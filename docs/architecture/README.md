...
implementation, `write_text` and `edit_text` (bounded-diff file edits) from `WorkspaceEditFunctions`, and
`repl_execute`, `repl_create`, `repl_list`, `repl_restart`, and `repl_close` from the local Deno
