...
internal static partial class WindowsJunction
{
    ...
    {
        // Track whether this call created the directory: a failure after creating it must not
        // leave an empty plain directory behind, because the caller treats a non-link at a
        // registered name as foreign and refuses to replace it, permanently wedging the name.
        ...
        {
            if (created) TryRemoveDirectory(linkPath);
            throw;
        }
    }
    ...
    private static void TryRemoveDirectory(string linkPath)
    {
        try
        {
        ...
        {
            // Best effort: the caller reports the original failure either way.
        }
    }
